...
% h = @(x) (x);
h = @(x,u,y_dd) [x(1); x(4)+x(5)-x(2); (y_dd-x(2))/Ts];

sigma_t = @(x) ti*x(5)*sqrt((1-a(x)^2)/(1-a(x))^2);
sigma_m = sqrt(Ts*q);
Q = @(x) 0.1*diag([0; 0; 0; x(5)^2*pi^2*sigma_t(x)^2/(2*L)^2; sigma_m^2]);
R = 0.1*eye(Yk,Yk);
