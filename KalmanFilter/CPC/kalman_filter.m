...
%% MODEL
w_p = @(x) x(14)*pi/(2*W.L); % Kaimal spectrum
ve = @(x) x(14) + x(13); % Effective wind speed at hub height
vr = @(x) ve(x) - x(3); % Relative wind velocity seen by the rotor

lamb = @(x) (x(1)*Ae.Rr-x(9))/(vr(x)-x(7)); % Lambda
...
R = diag(R); % Covariance matrix of measurement noise
...
%% True Values (Runge-Kutta 4th order method)
% Initialize matrices
...
[xt,yt] = RK4(f,xt,h,yt,u_b,N,n,v,Ts); % Runge-Kutta 4th order method
