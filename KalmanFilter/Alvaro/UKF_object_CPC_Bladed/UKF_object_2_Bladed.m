...
ukf = unscentedKalmanFilter(...
    @StateFcnDiscrete2Bladed,... % State transition function
    @MeasurementFcn2Bladed,... % Measurement function
    initialStateGuess,...
...
[~,xt] = ode45(@(t,x) odeWindTurbine2Bladed(t,x,u_b,d_b,n,timeVector,Ac,Ae,B,D,To,cp_l,ct_l,lambdaVec,pitchVec), timeVector, x_i);
xt = xt';
...
for k = 1:N
    yt(:,k) = MeasurementFcn2Bladed(xt(:,k),d_b(:,k),B,D,To) + v(:,k);
end
...
for k=1:N
    ...
    e(k,:) = yMeas(k,:) - MeasurementFcn2Bladed(ukf.State,d_b(:,k)',B,D,To)'; % ukf.State is x[k|k-1] at this point
    % Incorporate the measurements at time k into the state estimates by
    % using the "correct" command. This updates the State and StateCovariance
    % properties of the filter to contain x[k|k] and P[k|k]. These values
    % are also produced as the output of the "correct" command.
    [xCorrectedUKF(k,:), PCorrected(k,:,:)] = correct(ukf,yMeas(k,:),d_b(:,k)',B,D,To);
    % Predict the states at next time step, k+1. This updates the State and
    % StateCovariance properties of the filter to contain x[k+1|k] and
    % P[k+1|k]. These will be utilized by the filter at the next time step.
    predict(ukf,u_b(:,k)',d_b(:,k)',Ac,Ae,B,D,To,cp_l,ct_l,lambdaVec,pitchVec);
end
