...
xeq = x_i;
xmpc = zeros(Lk, N*n_int); % Initialize size of true state for all k
xmpc(:,1) = x_i; % Set true initial state
ympc = zeros(16, N*n_int); % Initialize size of true state for all k
...
for k=1:(N-1)
    ...
    for iter=1:n_int
        ...
        if iter == 1
            ...
            xeq(:,i) = xeq(:,i-1) + (x_tv(:,k+1) - x_tv(:,k))/n_int;
        end  
    ...
    A1 = [zeros(1,11), -a1*ones(1,3);
        ...
        d10(xeq(:,i),0), d3, d4(xeq(:,i),0), zeros(1,2), -d1, zeros(1,2), d2(xeq(:,i),0), zeros(1,5);
        d10(xeq(:,i),1), d3, d4(xeq(:,i),1), zeros(1,3), -d1, zeros(1,2), d2(xeq(:,i),1), zeros(1,4);
        d10(xeq(:,i),2), d3, d4(xeq(:,i),2), zeros(1,4), -d1, zeros(1,2), d2(xeq(:,i),2), zeros(1,3);
        zeros(3,14)];
    ...
    A2 = [zeros(1,9), -a2, zeros(1,3);
        ...
        zeros(3,13);
        d8(xeq(:,i),0), zeros(1,2), d7(xeq(:,i),0), zeros(1,6), d6(xeq(:,i),0), d5(xeq(:,i),0), d9(xeq(:,i),0);
        0, d8(xeq(:,i),1), zeros(1,2), d7(xeq(:,i),1), zeros(1,5), d6(xeq(:,i),1), d5(xeq(:,i),1), d9(xeq(:,i),1);
        0, 0, d8(xeq(:,i),2), zeros(1,2), d7(xeq(:,i),2), zeros(1,4), d6(xeq(:,i),2), d5(xeq(:,i),2), d9(xeq(:,i),2);
        eye(3), zeros(3,10)];
    
    A3 = [-e9(xeq(:,i),0), 0, -e7(xeq(:,i),0), e3, e4, zeros(1,3), -e8(xeq(:,i),0), 0, 0, -e1, 0, 0;
        -e9(xeq(:,i),1), 0, -e7(xeq(:,i),1), e3, e4, zeros(1,4), -e8(xeq(:,i),1), 0, 0, -e1, 0;
        -e9(xeq(:,i),2), 0, -e7(xeq(:,i),2), e3, e4, zeros(1,5), -e8(xeq(:,i),2), 0, 0, -e1;
        zeros(6,14);
        g2(xeq(:,i),uprev_mpc), zeros(1,13);
%         zeros(1,14);
        zeros(2,14);
        1, zeros(1,13)];
    
    A4 = [-e2(xeq(:,i),0), zeros(1,2), -e11(xeq(:,i),0), zeros(1,6), -e6(xeq(:,i),0), -e5(xeq(:,i),0), -e10(xeq(:,i),0);
        0, -e2(xeq(:,i),1), zeros(1,2), -e11(xeq(:,i),1), zeros(1,5), -e6(xeq(:,i),1), -e5(xeq(:,i),1), -e10(xeq(:,i),1);
        0, 0, -e2(xeq(:,i),2), zeros(1,2), -e11(xeq(:,i),2), zeros(1,4), -e6(xeq(:,i),2), -e5(xeq(:,i),2), -e10(xeq(:,i),2);
...
%         zeros(1,Lk-4), g3(xeq), zeros(1,3)]';
    ...
    Cmpc = [1, zeros(1,Lk-1);
        ...
        q2(xeq(:,i)), zeros(1,Lk-5), q1(xeq(:,i)), zeros(1,3)];
    %     xmpc(:,k+1) = xmpc(:,k) + Ts*(Ampc*xmpc(:,k) + Bmpc*u_b(:,k));
    %     ympc(:,k+1) = Cmpc*xmpc(:,k+1);
    
    k_1 = Ampc*xmpc(:,i) + Bmpc*u_b(:,k);
    k_2 = Ampc*(xmpc(:,i)+0.5*Ts*k_1) + Bmpc*(u_b(:,k));
    k_3 = Ampc*(xmpc(:,i)+0.5*Ts*k_2) + Bmpc*(u_b(:,k));
    k_4 = Ampc*(xmpc(:,i)+Ts*k_3) + Bmpc*(u_b(:,k));
    xmpc(:,i+1) = xmpc(:,i) + (1/6)*(k_1+2*k_2+2*k_3+k_4)*Ts;
%     xmpc(:,k+1) = (eye(Lk) + Ts*Ampc)*xmpc(:,k) + Ts*Bmpc*u_b(:,k);
    ympc(:,i+1) = Cmpc*xmpc(:,i+1);
...
% true_plots(Lk,yt,x_tv,xmpc,x_ul,x_vl,var_names,t)
