...
v = sqrt(R)*randn(Yk, N*n_int); % Generate random measurement noise (from assumed R)
% v = zeros(Yk, N);

%% Initialization
% Initialize matrices
x_tv = zeros(Lk, N*n_int); % Initialize size of true state for all k
x_tv(:,1) = x_i; % Set true initial state
yt = zeros(Yk, N*n_int); % Initialize size of output vector for all k

% Initialize state and covariance
x_kf = zeros(Lk, N*n_int); % Initialize size of state estimate for all k
x_kf(:,1) = x_i;
...
%         zeros(1,Lk)]';
        zeros(1,Lk-4), g3(xmpc(:,k)), zeros(1,3)]';
    %     Bmpc = [zeros(3,Lk-7), f1*eye(3), zeros(3,4);
