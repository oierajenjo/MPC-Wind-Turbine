...
dvri_dazim = @(x,i) x(26)*(To.r^2*(Ae.Rr^2*(sin(x(27)+2*pi*i/3))^2 + To.xh^2)*...
    (2*Ae.Rr^2*sin(x(27)+2*pi*i/3)*cos(x(27)+2*pi*i/3)))/(Ae.Rr^2*(sin(x(27)+2*pi*i/3))^2 + To.xh^2)...
...
d1 = B.kx/B.m; % dx_bi
d2 = @(x,i) Ae.rho*Ae.Ar*(dct_dlamb*dlamb_dxdotbi(x,i)*vri_eq(x,i)^2 - 2*cti(x,i)*vri_eq(x,i) - B.cx/B.m)/(6*B.m); % dxdot_bi
d3 = d1; % dx_t
d4 = @(x,i) Ae.rho*Ae.Ar*(dct_dlamb*dlamb_dxdott(x,i)*vri_eq(x,i)^2 - 2*cti(x,i)*vri_eq(x,i) - B.cx/B.m)/(6*B.m); % dxdot_t
d5 = @(x,i) Ae.rho*Ae.Ar*(dct_dlamb*dlamb_dvm(x,i)*vri_eq(x,i)^2 + 2*cti(x,i)*ws_ts(x,i)*vri_eq(x,i))/(6*B.m); % dv_m
d6 = @(x,i) Ae.rho*Ae.Ar*(dct_dlamb*dlamb_dvt(x,i)*vri_eq(x,i)^2 + 2*cti(x,i)*vri_eq(x,i))/(6*B.m); % dv_t
...
d9 = @(x,i) Ae.rho*Ae.Ar*(2*cti(x,i)*dvri_dazim(x,i)*vri_eq(x,i) + dct_dlamb*dlamb_dazim(x,i)*vri_eq(x,i)^2)/(6*B.m); % dazim
...
e3 = e1; % dy_t
e4 = B.cy/B.m; % dydot_t
e5 = @(x,i) Ae.rho*Ae.Ar*(dcp_dlamb*dlamb_dvm(x,i)*vri_eq(x,i)^3 + 3*cpi(x,i)*ws_ts(x,i)*vri_eq(x,i)^2)/(4*x(1)*B.l); % dv_m
e6 = @(x,i) Ae.rho*Ae.Ar*(dcp_dlamb*dlamb_dvt(x,i)*vri_eq(x,i)^3 + 3*cpi(x,i)*vri_eq(x,i)^2)/(4*x(1)*B.l); % dv_t
e7 = @(x,i) Ae.rho*Ae.Ar*(dcp_dlamb*dlamb_dxdott(x,i)*vri_eq(x,i)^3 - 3*cpi(x,i)*vri_eq(x,i)^2)/(4*x(1)*B.l); % dxdot_t
e8 = @(x,i) Ae.rho*Ae.Ar*(dcp_dlamb*dlamb_dxdotbi(x,i)*vri_eq(x,i)^3 - 3*cpi(x,i)*vri_eq(x,i)^2)/(4*x(1)*B.l); % dxdot_bi
e9 = @(x,i) Ae.rho*Ae.Ar*vri_eq(x,i)^3*(dcp_dlamb*dlamb_dwr(x,i)-cpi(x,i)/x(1))/(4*x(1)*B.l); % dw_r
e10 = @(x,i) Ae.rho*Ae.Ar*(3*cpi(x,i)*dvri_dazim(x,i)*vri_eq(x,i)^2 + dcp_dlamb*dlamb_dazim(x,i)*vri_eq(x,i)^3)/(4*x(1)*B.l); % dazim
