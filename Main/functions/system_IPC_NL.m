function [f,h,hz,Q,R] = system_IPC_NL(var,ct_l,cp_l,lambdaVec,pitchVec,Lk)
...
f = @(x,u,Ts) [f1(x); f2(x); f3(x); f4(x); f5(x); f6(x); f7(x);...
...
hz = @(x,u,Ts) [x(1); x(3); x(5); x(9); x(10);
    x(11); x(15); x(16); x(17); (x(1)*Ae.Rr-x(15))/vri(x,0); (x(1)*Ae.Rr-x(16))/vri(x,1);...
    (x(1)*Ae.Rr-x(17))/vri(x,2); x(18); x(19); x(20); x(21); x(22); x(23); D.eta*x(24)*x(1)];
